...
def _get_row_labels(n_obs: int, valid_mask: np.ndarray) -> list[str]:
    ...
    def _try_col(col: str) -> list[str] | None:
        arr = _store.dataset[col]
        if arr.dtype.kind not in ('U', 'S', 'O'):
            return None
        if col in _store.datetime_cols:
            return None
        n = len(arr)
        if n == 0:
            return None
        uniq_ratio = len(np.unique(arr)) / max(n, 1)
        if uniq_ratio < 0.50:
            return None
        max_len = max((len(str(v)) for v in arr[:100] if v), default=0)
        if max_len > 60:
            return None
        return arr[valid_mask].astype(str).tolist()

    id_keywords = ("id", "code", "label", "name", "key", "index")
    priority, rest = [], []
