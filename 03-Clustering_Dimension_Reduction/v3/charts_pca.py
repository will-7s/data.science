...
All layout overrides strip conflicting keys before spreading.
"""
...
from charts_common import layout, THETA, BLUE, RED, GREEN, GREY, ORANGE, VAR_COLORS
...
def scree_plot(result: dict) -> go.Figure:
    ...
    fig.add_trace(go.Bar(
        ...
        name="Individual (%)",
        marker_color=[BLUE if i < n_opt else GREY for i in range(n_comp)],
        opacity=0.85,
    ...
    fig.add_hline(
        y=70, line_dash="dot", line_color=GREEN, line_width=1,
        secondary_y=True,
        annotation_text="70 %", annotation_font_size=9,
        annotation_font_color=GREEN, annotation_position="right",
    )
...
def correlation_circle(result: dict, pc_x: int = 0, pc_y: int = 1,
    ...
    for j, var in enumerate(variables):
        ...
        color   = VAR_COLORS[j % len(VAR_COLORS)]
...
def individuals_plane(result: dict, pc_x: int = 0, pc_y: int = 1,
    ...
    if top_contrib_n > 0:
        ...
        fig.add_trace(go.Scatter(
            ...
            mode="markers+text",
            marker=dict(size=9, color=RED, symbol="circle-open", line_width=1.5),
    ...
    fig.add_vline(x=0, line_dash="dot", line_color=GREY, line_width=0.8)
...
def contributions_bar(result: dict, pc_idx: int) -> go.Figure:
    ...
    scos2 = cos2[order]
    scorr = corr[order]
    colors = [BLUE if v >= threshold else GREY for v in sc]
    ...
    fig.update_layout(
        **layout(height=h, margin=dict(l=120, r=60, t=50, b=50)),
...
def cos2_heatmap(result: dict, n_pc: int = 5) -> go.Figure:
    ...
    fig.update_layout(
        **layout(height=h, margin=dict(l=130, r=50, t=50, b=80)),
