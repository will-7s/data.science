...
def circle_interpretation_panel(result: dict, pc_x: int, pc_y: int,
    ...
    for j in range(n_vars):
        ...
        cyj = float(cy[j])
        if csj >= 0.6:
            well_repr.append((variables[j], csj))
        elif csj < 0.3:
            poor_repr.append((variables[j], csj))
        if cxj > 0.4:
            pos_x.append((variables[j], cxj))
        elif cxj < -0.4:
            neg_x.append((variables[j], cxj))
        if cyj > 0.4:
            pos_y.append((variables[j], cyj))
        elif cyj < -0.4:
            neg_y.append((variables[j], cyj))
    pos_x.sort(key=lambda x: -x[1])
    neg_x.sort(key=lambda x: x[1])
    pos_y.sort(key=lambda x: -x[1])
    neg_y.sort(key=lambda x: x[1])

    corr_pairs = []
    opp_pairs = []
    for j in range(n_vars):
        csj = cs[j]
        if csj < cos2_quality_min:
            continue
        for k in range(j + 1, n_vars):
            if cs[k] < cos2_quality_min:
                continue
...
def contributions_interpretation_panel(result: dict, pc_idx: int) -> html.Div:
    ...
    for j in above:
        c = float(corr[j, pc])
        contrib_val = float(contrib[j, pc])
        (pos_drivers if c > 0 else neg_drivers).append((variables[j], c, contrib_val))
    pos_drivers.sort(key=lambda x: -x[2])
    neg_drivers.sort(key=lambda x: -x[2])

    cos2_pc = cos2[:, pc]
    corr_pc = corr[:, pc]
    good = np.where(cos2_pc >= 0.6)[0]
    redundant_pairs = []
    for idx_j in range(len(good)):
        j = good[idx_j]
        cos2_j = float(cos2_pc[j])
        for idx_l in range(idx_j + 1, len(good)):
            l = good[idx_l]
            same_sign = (corr_pc[j] * corr_pc[l]) > 0
            redundant_pairs.append((
                variables[j], variables[l], cos2_j, float(cos2_pc[l]),
