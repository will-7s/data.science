...
def plot_conversion_rates(stats: dict, save_path: Path = None):
...
def plot_confidence_intervals(stats: dict, save_path: Path = None):
    fig, ax = plt.subplots(figsize=(8, 5))
    groups = [stats["control"]["label"].capitalize(), stats["treatment"]["label"].capitalize()]
...
def plot_bayesian_posteriors(bayes_results: dict, labels: tuple = None, save_path: Path = None):
    ...
    axes[0].set_ylabel("Posterior Mean Conversion Rate", fontsize=12)
...
def plot_segmentation(seg_results: dict, save_path: Path = None):
    rows = []
...
def plot_power_analysis(mde: float, power_obs: float, save_path: Path = None):
...
def plot_daily_trends(daily: pd.DataFrame, save_path: Path = None):
