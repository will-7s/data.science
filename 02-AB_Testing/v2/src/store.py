...
def _is_id_column(arr: np.ndarray, col_name: str) -> bool:
    """FIX: exclude datetime columns from ID candidates by requiring integer dtype."""
    id_keywords = {"id", "user", "uuid", "key", "code", "num"}
    name_lower = col_name.lower()
    name_match = any(k in name_lower for k in id_keywords)

    # Datetime-like columns are never IDs even if n_unique is high
    ...
    # High-cardinality integer/string columns are candidate IDs
    if arr.dtype.kind in _INT_KINDS or arr.dtype.kind in _STR_KINDS:
        n_uniq = len(np.unique(arr))
...
def reset(df: pd.DataFrame) -> None:
    """Rebuild all derived state from a fresh DataFrame."""
    ...
    for col in all_cols:
        ...
        if _is_id_column(arr, col):   # FIX: datetime excluded
