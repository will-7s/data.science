...
def run_temporal_analysis(prepared: dict) -> dict:
    # Agrège les conversions par jour et par groupe, puis calcule la tendance
    # linéaire (Pearson r pondéré par √n) pour chaque groupe.
    # Utile pour détecter :
    #   - Novelty effect : effet fort au début qui s'estompe.
    #   - Effet d'apprentissage : effet qui se renforce avec le temps.
    #   - Saisonnalité : patterns hebdomadaires (intégrée dans la régression
    #     logistique enrichie via les features hour/weekend).
    ...
    for grp_val in np.unique(prepared["group"]):
        ...
        try:
            ...
            warnings.warn(
                f"Temporal trend computation failed for {grp_val}: {exc}",
                RuntimeWarning,
                stacklevel=2,
            )
            trends[str(grp_val)] = {
    ...
    return {"daily_data": daily, "trends": trends, "error": None}
