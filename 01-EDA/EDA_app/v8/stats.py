...
------------------------
For n > config.SUBSAMPLE_THRESHOLD, normality tests suffer from over-rejection:
any trivial deviation from normality becomes significant. To produce
practically meaningful p-values, we:
  1. Draw config.SUBSAMPLE_REPS (10) stratified subsamples of size config.SUBSAMPLE_N.
...
def _sp() -> type:
    """Lazy-loaded scipy.stats module."""
    from scipy import stats as _m
    return _m
...
def run_normality_battery(arr: np.ndarray) -> tuple[list[NormalityResult], dict]:
    ...
    subsample when n > config.SUBSAMPLE_THRESHOLD to avoid over-rejection.
    """
    clean  = drop_nan(arr)
    n_full = len(clean)
    ...
    if n_full <= config.SUBSAMPLE_THRESHOLD:
        results = _run_five_tests(clean)
    else:
        # One argsort for all K subsamples — O(n log n) paid once, not K times
        mat   = _subsample_matrix(clean, config.SUBSAMPLE_REPS, config.SUBSAMPLE_N,
                                  seed=config.SUBSAMPLE_SEED)
        runs  = [_run_five_tests(mat[k]) for k in range(config.SUBSAMPLE_REPS)]
        results = _aggregate_results(runs)
        # Append subsampling context note to every result
        sub_note = (f"Tests run on {config.SUBSAMPLE_REPS} stratified subsamples "
                    f"of n = {config.SUBSAMPLE_N:,} (full n = {n_full:,}). "
        ...
        shape.update({
            ...
            'subsample_k': config.SUBSAMPLE_REPS,
        })
...
def _kolmogorov_smirnov(c: np.ndarray) -> NormalityResult:
    ...
    d, p = _sp().kstest(c, 'norm', args=(mu, sigma))
    ok = p > config.ALPHA
...
def _anderson_darling(c: np.ndarray) -> NormalityResult:
    ...
    try:
        ...
        res  = _sp().anderson(c, dist='norm', method='interpolate')
...
def group_normality_report(
    ...
    """
    Shapiro-Wilk per group with stratified subsampling when n > config.SUBSAMPLE_THRESHOLD.

    Rules
    -----
    n < 3                        → cannot run
    3 ≤ n ≤ config.SUBSAMPLE_THRESHOLD → Shapiro-Wilk directly
    n > config.SUBSAMPLE_THRESHOLD     → config.SUBSAMPLE_REPS subsamples, majority vote
    ...
    for arr, lbl in zip(groups, labels):
        ...
        # _sp().describe: single pass — n, mean, variance, skewness, kurtosis
        if n >= 3:
            desc = _sp().describe(clean)
        ...
        if n <= config.SUBSAMPLE_THRESHOLD:
            w, p = _sp().shapiro(clean)
            ok   = bool(p > config.ALPHA)
            ...
            mat_g  = _subsample_matrix(clean, config.SUBSAMPLE_REPS, config.SUBSAMPLE_N,
                                       seed=config.SUBSAMPLE_SEED)
            sub_ps = np.array([_sp().shapiro(mat_g[k])[1]
                                for k in range(config.SUBSAMPLE_REPS)])
            med_p  = float(np.median(sub_ps))
            ok     = med_p > config.ALPHA
            note   = (f"{'Normal' if ok else 'Non-normal'}  "
                      f"(median p = {med_p:.4f} over {config.SUBSAMPLE_REPS} "
                      f"subsamples of n = {config.SUBSAMPLE_N:,})")
...
def normality_test(arr: np.ndarray, max_n: int = 5000):
    """Shapiro-Wilk with random subsampling for large arrays."""
    clean = drop_nan(arr)
    if len(clean) < 3:
        return None, None
    if len(clean) > max_n:
        rng = np.random.default_rng(0)
        clean = rng.choice(clean, max_n, replace=False)
    return _sp().shapiro(clean)


def normality_label(p: Optional[float]) -> str:
    if p is None:
        return "Insufficient data"
    return f"Normal (p = {p:.3f})" if p > config.ALPHA else f"Non-normal (p = {p:.3f})"


# ── Group statistics ──────────────────────────────────────────────────────────

def group_means(num_arr: np.ndarray, cat_arr: np.ndarray) -> dict[str, float]:
    cat_uniq, cat_codes = np.unique(cat_arr, return_inverse=True)
    ...
    return {
        str(c): float(drop_nan(g).mean())
        for c, g in zip(cat_uniq, groups)
        if drop_nan(g).size > 0
    }
...
def _test_num_num(a, b, v1, v2) -> dict:
    ...
    rp, pp = _sp().pearsonr(a, b)
    rs, ps = _sp().spearmanr(a, b)
    rk, pk = _sp().kendalltau(a, b)
    _, p1  = normality_test(a)
    _, p2  = normality_test(b)
    both   = (p1 or 0) > config.ALPHA and (p2 or 0) > config.ALPHA
    rec    = ("Pearson (both normal)" if both
              else "Kendall (small sample)" if len(a) < 30
              else "Spearman (non-normal)")
    sig    = lambda p: "✓ sig." if p < config.ALPHA else "✗ n.s."

    def _shape(arr, label, p_sw):
        # _sp().describe: single pass for n, mean, var, skew, kurt
        desc     = _sp().describe(arr)
        skew     = float(desc.skewness)
        kurt     = float(desc.kurtosis)
        norm_sym = "✓" if (p_sw or 0) > config.ALPHA else "✗"
        return [
            f"── Normality — {label} ──",
...
def _test_num_cat(num, cat) -> dict:
    cats_uniq, cats_codes = np.unique(cat, return_inverse=True)
    sort_idx   = np.argsort(cats_codes, kind='stable')
    num_sorted = num[sort_idx]
    cod_sorted = cats_codes[sort_idx]
    split_pts  = np.where(np.diff(cod_sorted))[0] + 1
    groups     = [drop_nan(g) for g in np.split(num_sorted, split_pts)]
    cats       = cats_uniq
    valid      = [g for g in groups if len(g) >= 2]
    if len(valid) < 2:
        return {"test_name": "Error", "results": ["Need ≥ 2 groups with ≥ 2 observations."]}

    lev_w, lev_p = _sp().levene(*valid)
    homo         = lev_p > config.ALPHA
    f_stat, ap   = _sp().f_oneway(*valid)
    h_stat, kp   = _sp().kruskal(*valid)
    sig          = lambda p: "✓ sig." if p < config.ALPHA else "✗ n.s."
    ...
    if len(valid) == 2:
        g1, g2 = valid
        t_stu, tp_stu = _sp().ttest_ind(g1, g2, equal_var=True)
        t_wel, tp_wel = _sp().ttest_ind(g1, g2, equal_var=False)
        u_stat, up    = _sp().mannwhitneyu(g1, g2, alternative='two-sided')
        n1, n2        = len(g1), len(g2)
...
def _test_cat_cat(a, b) -> dict:
    ...
    ct     = np.bincount(flat, minlength=k * r).reshape(k, r).astype(int)
    n      = ct.sum()

    chi2, p_chi, dof, exp = _sp().chi2_contingency(ct)
    min_exp   = exp.min()
    low_pct   = (exp < 5).mean() * 100
    chi_valid = (min_exp >= 1) and (low_pct <= 20)
    sig_chi   = "✓ sig." if p_chi < config.ALPHA else "✗ n.s."
    ...
    if use_fisher:
        try:
            res_fisher = _sp().fisher_exact(ct)
            p_f   = res_fisher.pvalue if hasattr(res_fisher, 'pvalue') else res_fisher[1]
            sig_f = "✓ sig." if p_f < config.ALPHA else "✗ n.s."
            results += [
                "",
