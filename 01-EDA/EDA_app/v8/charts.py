...
def _go():
    """Lazy-loaded plotly.graph_objects."""
    import plotly.graph_objects as _m
    return _m


def _px():
    """Lazy-loaded plotly.express."""
    import plotly.express as _m
    return _m
...
_LAYOUT = dict(
    ...
    margin=dict(l=54, r=44, t=28, b=48),
...
_SCATTER_MAX = config.SCATTER_MAX
_BAR_MAX_CATS = config.BAR_MAX_CATS
_PIE_MAX_CATS = config.PIE_MAX_CATS


def histogram(arr: np.ndarray, col: str) -> _go().Figure:
    clean = drop_nan(arr)
    if clean.size == 0:
        return empty()
...
def boxplot(arr: np.ndarray, col: str) -> _go().Figure:
    clean = drop_nan(arr)
    if clean.size == 0:
        return empty()
    bp  = 'suspectedoutliers' if len(clean) > 50_000 else 'outliers'
    fig = _go().Figure(_go().Box(
        y=clean, name=col,
...
def bar_categorical(arr: np.ndarray, col: str) -> _go().Figure:
...
def pie_categorical(arr: np.ndarray, col: str) -> _go().Figure:
    vals, cnts = np.unique(arr, return_counts=True)
    order      = np.argsort(cnts)[::-1]
    sv         = vals[order].astype(str)
    sc         = cnts[order]
...
def scatter(x: np.ndarray, y: np.ndarray, xl: str, yl: str,
            sampled: bool = False, scatter_max: int | None = None) -> _go().Figure:
    ...
    xm, ym = xm[fin], ym[fin]
    if len(xm) < 2:
        return empty('Not enough finite points')

    note = ''
    limit = scatter_max if scatter_max is not None else _SCATTER_MAX
...
def correlation_heatmap(mat: np.ndarray, cols: list[str]) -> _go().Figure:
    n = len(cols)
    fig = _px().imshow(
        mat, x=cols, y=cols,
...
def empty(msg: str = 'No data') -> _go().Figure:
    fig = _go().Figure()
